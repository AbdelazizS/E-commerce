  ...
  <Container>
    <div class="py-8 mb-8">
      <div class="grid md:grid-cols-2 md:gap-4 gap-6 md:gap-y-12" v-if="favouriteItems?.length > 0">
        <div v-for="item in favouriteItems" :key="item.id" class="bg-card shadow-md border max-w-lg p-4 relative">
          <div class="flex items-center gap-4 md:gap-6 items-">
            <div class="w-36 h-26 md:w-40 md:h-40">
              <img src="/src/assets/pc.png" alt="product-image" class="w-full rounded-lg h-full object-cover" />
            </div>
